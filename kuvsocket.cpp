...
void KUVSocket::ProcessReadEvent(uv_stream_t* pReader, ssize_t nRead, uv_buf_t& buf)
{
	...
	while (true)
	{
		if (pData->Reader.nRemainSize == 0)
		{
			...
			{
				fprintf_s(stdout, "KUVSocket::ProcessSendEvent Fail, the Recv Buff Size is less than Header Size, %d, %d",
			...
			{
				fprintf_s(stdout, "KUVSocket::ProcessReadEvent Fail, RecvBuff %d is less than RemainSize %d, Close the connect",
...
void KUVSocket::OnConnectionRead(uv_stream_t* pHandle, ssize_t nread, uv_buf_t buf)
{
	...
	if (nread < 0)
	{
		...
		{
			fprintf_s(stdout, "some one read finish, there is no buf for read");
