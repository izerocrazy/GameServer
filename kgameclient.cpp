...
KGameClient* KGameClient::m_Instance = NULL;
...
KGameClient* KGameClient::GetInstance()
{
	return m_Instance;
}

BOOL KGameClient::CreateClient(char* szIP, int nPort)
{
	BOOL bRet = TRUE;
	...
	m_Instance = new KGameClient;

	// bind func
	SetReadCallback(KGameClient::OnReadCallBack);

	m_Instance->m_hServerHandle = CreateSocket(1, szIP, nPort);
...
void KGameClient::OnReadCallBack(H_CONNECTION handle, char* szRead, int nLen)
{
	fprintf_s(stdout, "data:%s", szRead);

	KGameClient* pInstance = KGameClient::GetInstance();
	if (pInstance)
	{
...
BOOL KGameClient::ProcessPacket(H_CONNECTION handle, char* szRead, int nLen)
{
	BOOL bRet = FALSE;
